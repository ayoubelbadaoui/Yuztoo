...
class _DataPrivacyScreenState extends ConsumerState<DataPrivacyScreen> {
  ...
  // Calm entry-point. The actual destructive treatment (red icon, red
  // confirm button, "irréversible" copy) lives in [_DeleteConfirmSheet] —
  // exactly at the moment the user commits to it, never before. Earlier
  // versions painted the whole row red which scared users away from a
  // legitimate, GDPR-mandated action.
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          // Reached from BOTH the client profile (Sécurité &
          // Confidentialité) and the merchant settings, so the copy stays
          // generic — mentioning "vos clients, promotions" here would be
          // wrong for a client-only account.
          Text(
